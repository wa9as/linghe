...
def get_expert_indices(logits, n=4):
    assert logits.dtype == torch.float32
    bs, length, expert = logits.shape 
    logits = logits.view(bs*length, expert)
    double_logits = logits.to(torch.float64)*(1+torch.arange(expert, device=logits.device, dtype=torch.float64)*1e-12)
    double_top_values, top_indices = torch.topk(double_logits, n, dim=-1, largest=True, sorted=True)
...
def fp8_gmm(hs, wqs, smooth_scale, w_scale):
    ...
    for i in range(expert):
        ...
        output = torch._scaled_mm(x_q,
                                    wqs.view(torch.int8)[i].view(torch.float8_e4m3fn).t(),
...
def split_moe(hs, logits, wgs, wus, wds, n=4):
    # hs: [bs, length, dim]
    ...
    double_top_values, top_indices = torch.topk(double_logits, n, dim=-1, largest=True, sorted=True)
    mask_logits = torch.where(double_logits>=double_top_values[:,-1:], logits, -10000.0)
    probs = torch.softmax(double_top_values.float(), -1).to(hs.dtype)
    expert_indices = []
    outputs = []
    for i in range(expert):
        expert_index = torch.where(mask_logits[:,i]>-5000)[0]
...
def naive_fp8_moe(hs, logits, wfns, vfns, n=4):
    # hs: [bs, length, dim]
    # logits: [bs, length, expert]
    # w1s: [expert, im, dim]
    assert logits.dtype == torch.float32
    ...
    pads = triton_batch_slice_and_pad(hs, expert_indices, expert=expert, block=128)
    for i in range(expert):
        h = pads[i]
        gus = wfns[i](h)
        act = torch.nn.functional.silu(gus[:,:ims])*gus[:,ims:]
    ...
    gather_indices = torch.argsort(float_expert_indices, dim=-1, descending=False, stable=False)
    outputs = outputs[gather_indices]
    outputs = outputs.view(bs * length, n, dim)
    outputs = torch.einsum("bno,bn->bo",outputs,probs)
    return outputs.view(bs, length, dim)
...
def fused_fp8_moe_forward(hs, logits, ws, vs, n=4):
    # hs: [bs, length, dim]
    # logits: [bs, length, expert]
    # w1s: [expert, im, dim]
    assert logits.dtype == torch.float32
    ...
    x_q,w_q,x_scale,w_scale,smooth_scale = triton_smooth_quant_nt(hs, ws.view(expert*ims*2, dim))
    w_q = w_q.view(expert, ims*2, dim)
    w_scale = w_scale.view(expert, ims*2)
    pad_xs = triton_batch_slice_and_pad(x_q, expert_indices, expert=expert, block=128)
    pad_x_scales = triton_batch_slice_and_pad(x_scale, expert_indices, expert=expert, block=128)
    for i in range(expert):
        h = pad_xs[i]
        gus = torch._scaled_mm(h,
                                    w_q.view(torch.int8)[i].view(torch.float8_e4m3fn).t(),
...
def slide_fp8_moe_forward(hs, logits, ws, w_scales, vs, v_scales, w_smooth_scale, v_smooth_scale, n=4):
    # hs: [bs, length, dim]
    # logits: [bs, length, expert]
    # w1s: [expert, im, dim]
    assert logits.dtype == torch.float32
    ...
    for i in range(expert):
        h = pad_xs[i]
        gus = torch._scaled_mm(h,
                                    ws.view(torch.int8)[i].view(torch.float8_e4m3fn).t(),
                                    scale_a=pad_x_scales[i],
                                    scale_b=w_scale[i:i+1],
                                    out_dtype=hs.dtype,
                                    use_fast_accum=True)
        ...
        out = torch._scaled_mm(a_q,
                                    vs.view(torch.int8)[i].view(torch.float8_e4m3fn).t(),
                                    scale_a=a_scale,
...
class GLMTopNRouter(torch.nn.Module):
    ...
    def _route_tokens(self, router_logits: torch.Tensor):
        router_probs = torch.nn.functional.softmax(router_logits, dim=-1, dtype=torch.float)
...
if __name__ == '__main__':
    ...
    gate_weights = torch.randn((expert,ims,dim), dtype=dtype, device=device)
    up_weights = torch.randn((expert,ims,dim), dtype=dtype, device=device)
    down_weights = torch.randn((expert,dim, ims), dtype=dtype, device=device)
    ...
    expert_indices = get_expert_indices(logits, n=n_act)
    split_hidden_states = [hidden_states.view(bs*length,dim)[x] for x in expert_indices ]
    gate_up_scales = torch.rand((1, expert*ims*2), dtype=torch.float32, device=device)
    down_scales = torch.rand((1, expert*dim), dtype=torch.float32, device=device)
    gate_up_smooth_scale = torch.rand((dim, ), dtype=torch.float32, device=device)
    down_smooth_scale = torch.rand((ims, ), dtype=torch.float32, device=device)

    wfns = []
    vfns = []
    for i in range(expert):
        wfns.append(QuantLinear(in_features=dim, out_features=2*ims, dtype=dtype, device=device))
        vfns.append(QuantLinear(in_features=ims, out_features=dim, dtype=dtype, device=device))
    ...
    # print(expert*ims*2)
    # x_q,w_q,x_scale,w_scale,smooth_scale = triton_smooth_quant_nt(hidden_states.view(bs*length, dim), gate_up_weights.view(expert*ims*2, dim))
    # fp8_moe_forward(hidden_states, logits, gate_up_weights, down_weights, n=n_act)
    # slide_fp8_moe_forward(hidden_states, logits, gate_up_weights.to(torch.float8_e4m3fn), gate_up_scales, down_weights.to(torch.float8_e4m3fn), down_scales, gate_up_smooth_scale, down_smooth_scale, n=4)
    ...
    # benchmark_func(split_moe, hidden_states, logits, gate_weights, up_weights, down_weights, n=n_act, ref_flops=ref_flops, n_repeat=n_repeat)
    # ref_time = benchmark_func(moe, hidden_states, logits, gate_up_weights, down_weights, n=n_act, ref_flops=ref_flops, n_repeat=n_repeat)
    # benchmark_func(naive_fp8_moe, hidden_states, logits, wfns, vfns, n=n_act, ref_flops=ref_flops, ref_time=ref_time, n_repeat=n_repeat)
    # benchmark_func(fused_fp8_moe_forward, hidden_states, logits, gate_up_weights, down_weights, n=n_act, ref_flops=ref_flops, ref_time=ref_time,n_repeat=n_repeat)
    # benchmark_func(slide_fp8_moe_forward, hidden_states, logits,  gate_up_weights.to(torch.float8_e4m3fn), gate_up_scales, down_weights.to(torch.float8_e4m3fn), down_scales, gate_up_smooth_scale, down_smooth_scale, n=n_act, ref_flops=ref_flops, ref_time=ref_time,n_repeat=n_repeat)
