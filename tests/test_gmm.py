...
def moe_ref(hs, logits, ws, vs, n_act=4):
    ...
    double_top_values, top_indices = torch.topk(double_logits, n_act, dim=-1, largest=True, sorted=True)
    mask_logits = torch.where(double_logits>=double_top_values[:,-1:], logits, -10000.0)
    mask_probs = torch.softmax(mask_logits, -1).to(hs.dtype)  # [bs*length, expert]
    outputs = []
    for i in range(expert):
        gus = hs@ws[i].t()
...
def naive_fp8_moe(hs, logits, wfns, vfns, n_act=4):
    ...
    for i in range(expert):
        ...
        if n_token%128 != 0:
            ...
            expert_index = torch.nn.functional.pad(expert_index, (0,pad_token), "constant", 0)
        expert_prob = mask_probs[expert_index, i:i+1]
        h = hs[expert_index]
        gus = wfns[i](h)
        act = torch.nn.functional.silu(gus[:,:ims])*gus[:,ims:]
...
def slide_fp8_moe_forward(hs, logits, ws, w_scales, vs, v_scales, w_smooth_scale, v_smooth_scale, n_act=4):
    # hs: [bs, length, dim]
    # logits: [bs, length, expert]
    # w1s: [expert, im, dim]
    assert logits.dtype == torch.float32
...
if __name__ == '__main__':
    ...
    gate_weights = 0.1*torch.randn((expert,ims,dim), dtype=dtype, device=device)
    up_weights = 0.1*torch.randn((expert,ims,dim), dtype=dtype, device=device)
    down_weights = 0.1*torch.randn((expert,dim, ims), dtype=dtype, device=device)
    ...
    org_output = moe_ref(hidden_states, logits, gate_up_weights, down_weights, n_act=n_act)
