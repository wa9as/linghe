...
def torch_cross_entropy(logits, targets):
    ...
    losses = torch.nn.functional.cross_entropy(
        float_logits.view(-1, logits.size()[-1]), targets.view(-1), reduction="none"
    )
    loss = losses.sum()
    loss.backward()
...
def test_triton_softmax_cross_entropy(M=4096, N=157184, coef=1.0, bench=False):
    device = "cuda:0"
    logits = torch.randn(
        (M, N), dtype=torch.bfloat16, device=device, requires_grad=False
    )
    ...
    grad = triton_softmax_cross_entropy_backward(
        logits, targets, sum_exp, max_logit, input_grad, output_grad=logits
    )
    output_check(grad_ref.float(), grad.float(), mode="grad")
    if bench:
        benchmark_func(torch_cross_entropy, logits, targets, ref_bytes=M * N * 6)
        benchmark_func(
            triton_softmax_cross_entropy_forward, logits, targets, ref_bytes=M * N * 2
        )
        benchmark_func(
            triton_softmax_cross_entropy_backward,
            logits,
