...
def torch_smooth_permute_with_indices(grad_data, grad_scale, indices, smooth_scales, token_count_per_expert_list):
    M, N = grad_data.shape 
...
def test_triton_index_select_smooth_quant(M=4096, N=4096, n_experts=256, topk=8):
    device='cuda:0'
...
def test_triton_smooth_weighted_unpermute_with_indices_backward(M=4096, N=4096, n_experts=256, topk=8):
    device='cuda:0'
    reverse = True
    y = torch.randn((M, N), dtype=torch.bfloat16, device=device) 
    logits = torch.randn((M, n_experts), dtype=torch.float32, device=device) 
    ...
    y_q,y_scale,y_sum = triton_smooth_weighted_unpermute_with_indices_backward(y, tokens, smooth_scales, token_count_per_expert, indices, x_q=None, x_scale=None, reverse=reverse, round_scale=False)
    ...
    benchmark_func(triton_smooth_weighted_unpermute_with_indices_backward, y, tokens, smooth_scales, token_count_per_expert, indices, reverse=False, round_scale=False, n_repeat=n_repeat)
...
def test_triton_smooth_permute_with_mask_map(M=4096, N=4096, n_experts=32, topk=8):
    device='cuda:0'
    dtype = torch.bfloat16
    smooth_scales = 1+10*torch.rand((n_experts, N),device=device,dtype=torch.float32)
    logits = torch.randn((M,n_experts), dtype=torch.float32, device=device)
    ...
    y_q, y_scale = triton_smooth_unpermute_with_indices_backward(grad_data, grad_scale, smooth_scales, token_count_per_expert, indices, x_q=None, x_scale=None, reverse=False, round_scale=True)


    q_ref, scale_ref= torch_smooth_permute_with_indices(grad_data, grad_scale, indices, smooth_scales, token_count_per_expert_list)
    ...
    permuted_data, permuted_scale = triton_smooth_permute_with_mask_map(grad_data,row_id_map,grad_scale,M, n_experts, out_tokens,N,smooth_scales, reverse=False,round_scale=True)
    output_check(q_ref.float(), permuted_data.float(), 'data')
    output_check(scale_ref.float(), permuted_scale.float(), 'scale')

    benchmark_func(triton_smooth_unpermute_with_indices_backward, grad_data, grad_scale, smooth_scales, token_count_per_expert, indices, round_scale=True, n_repeat=100, ref_bytes=out_tokens*N*2)
    benchmark_func(triton_smooth_permute_with_mask_map, grad_data,row_id_map,grad_scale,M,n_experts,out_tokens,N,smooth_scales,reverse=False,round_scale=True, n_repeat=100, ref_bytes=out_tokens*N*2)


if __name__ == '__main__':

    test_triton_index_select_smooth_quant(M=4096, N=4096, n_experts=256, topk=8)
    test_triton_index_select_smooth_quant(M=4096, N=4096, n_experts=256, topk=8)
    test_triton_permute_with_mask_map(M=4096, N=4096, n_experts=256, topk=8)
