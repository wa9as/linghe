...
# optional dequant and smooth and quant
def torch_smooth_permute_with_indices(
    grad_data,
    grad_scale,
    indices,
    smooth_scales,
    ...
    for i, c in enumerate(token_count_per_expert_list):
        ...
        if grad_scale is not None:
            ...
            y_smooth = (
                data_slice.float().view(c, N // B, B) * scale_slice[:, :, None]
            ).view(c, N) / smooth_scales[i]
        else:
            y_smooth = data_slice.float() / smooth_scales[i]
...
def torch_batch_transpose_smooth_permute_with_indices(
    x_q,
    x_scale,
    org_smooth_scale,
    smooth_scales,
    ...
    for i, c in enumerate(token_count_per_expert_list):
        c = token_count_per_expert_list[i]
        if c == 0:
            y_scale = torch.zeros((DIM,), dtype=torch.float32, device=x_q.device)
        ...
        y_q, y_scale, y_max = torch_smooth_quant(
            y.t().contiguous(), smooth_scale, reverse=True, round_scale=round_scale
        )
        scale_refs.append(y_scale.view(-1))
...
def test_triton_smooth_weighted_permute_with_indices(
    M=4096, N=4096, n_experts=256, topk=8, round_scale=True, bench=False
):
    device = "cuda:0"
    reverse = True
    ...
    smooth_scales = 1 + 10 * torch.rand(
        (n_experts, N), device=device, dtype=torch.float32
    )
    probs, mask_map, token_count_per_expert, indices, row_id_map = torch_make_indices(
    ...
    y_q, y_scale, y_sum = triton_smooth_weighted_permute_with_indices(
        y,
        tokens,
        smooth_scales,
...
def test_triton_permute_with_mask_map(
    M=4096, N=4096, n_experts=256, topk=8, bench=False
):
    device = "cuda:0"
    dtype = torch.bfloat16
    ...
    x_out, scale_out, probs_out = triton_permute_with_mask_map(
        x, scales, probs, row_id_map, out_tokens, contiguous=True
    )
    output_check(x_out_ref, x_out, "x_out")
    output_check(scale_out_ref, scale_out, "scale_out")
    ...
    padded_out_tokens = sum(
        [(x + 15) // 16 * 16 for x in token_count_per_expert.tolist()]
    )
    x_out_ref = torch_select_with_padded_map_mask(
        x, round_row_id_map, padded_out_tokens
    )
    scale_out_ref = torch_select_with_padded_map_mask(
        scales, round_row_id_map, padded_out_tokens
    )
    prob_out_ref = torch_ravel_with_padded_map_mask(
        probs, round_row_id_map, padded_out_tokens
    )
    x_out, scale_out, probs_out = triton_permute_with_mask_map(
        x,
        scales,
...
def test_triton_smooth_permute_with_mask_map(
    M=4096, N=4096, n_experts=32, topk=8, round_scale=True, bench=False
):
    device = "cuda:0"
    dtype = torch.bfloat16
    smooth_scales = 1 + 10 * torch.rand(
    ...
    q_ref, scale_ref = torch_smooth_permute_with_indices(
        grad_data,
        grad_scale,
        indices,
        smooth_scales,
    ...
    y_q, y_scale = triton_smooth_permute_with_indices(
        grad_data,
        grad_scale,
        smooth_scales,
        token_count_per_expert,
...
def test_triton_batch_transpose_smooth_permute_with_indices(
    M=1024, N=2048, n_experts=32, topk=8, bench=False
):
    ...
    if True:
        ...
        logits[:, 0] -= 1000
        logits[:, 2] -= 100
        probs, mask_map, token_count_per_expert, indices, row_id_map = (
            torch_make_indices(logits, topk=topk, bias=-0.01)
        )
        ...
        smooth_scales = (
            torch.rand((out_tokens,), dtype=torch.float32, device=device) + 0.1
        )
    else:
    ...
    x_q_ref, x_scale_ref = torch_batch_transpose_smooth_permute_with_indices(
        x,
        scale,
        org_smooth_scale,
        smooth_scales,
    ...
    x_q, x_scale = triton_batch_transpose_smooth_permute_with_indices(
        x,
        scale,
        org_smooth_scale,
        smooth_scales,
    ...
    output_check(x_q_ref.float(), x_q.float(), "bf16.data")
    output_check(x_scale_ref.float(), x_scale.float(), "bf16.scale")
...
if __name__ == "__main__":
    ...
    test_triton_permute_with_mask_map(
        M=16384, N=2048, n_experts=32, topk=8, bench=False
    )
    ...
    test_triton_batch_transpose_smooth_permute_with_indices(
        M=16384, N=2048, n_experts=32, topk=2, bench=False
    )
    test_triton_batch_transpose_smooth_permute_with_indices(
        M=8192, N=4096, n_experts=32, topk=2, bench=False
    )
