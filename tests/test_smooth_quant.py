...
def torch_subrow_smooth_quant(
    ...
    if offset % N > 0:
        ...
        x_q_slice = torch.minimum(torch.maximum(x_smooth / scale, -limit), limit).to(
            torch.float8_e4m3fn
        )
...
def test_triton_smooth_quant(M=4096, N=4096, bench=False):
    ...
    x_q_ref, scales_ref, x_maxs_ref = torch_smooth_quant(
        x, smooth_scale, reverse=False, round_scale=True
    )

    x_q, x_scale, x_maxs = triton_smooth_quant(
        x, smooth_scale, reverse=False, round_scale=True, calibrate=True
    )
    output_check(x_q_ref.float(), x_q.float(), "triton_smooth_quant.data")
...
def test_triton_subrow_smooth_quant(M=4096, N=5120, offset=4096, size=16384):
    ...
    if offset % N > 0:
        k = N - offset % N
        output_check(
            x_q_ref.float().view(-1)[offset : offset + k],
    ...
    if (offset + size) % N > 0:
        k = (offset + size) % N
        output_check(
            x_q_ref.float().view(-1)[offset + size - k : offset + size],
...
def test_triton_transpose_smooth_quant(M=4096, N=4096, bench=False):
    ...
    transpose_smooth_scale = (
        torch.randn((M,), device=device, dtype=torch.float32).abs() * 10 + 1
    )
...
def test_triton_transpose_rescale_smooth_quant(M=4096, N=4096, round_scale=False):
    ...
    org_smooth_scale = (
        torch.randn((N,), device=device, dtype=torch.float32).abs() * 10 + 1
    )
    if round_scale:
        org_smooth_scale = torch.exp2(torch.ceil(torch.log2(org_smooth_scale)))
    transpose_smooth_scale = (
        torch.randn((M,), device=device, dtype=torch.float32).abs() + 0.1
    )
    if round_scale:
    ...
    yt_q_ref, yt_scale_ref = torch_rescale_quant(
        y_q,
        org_smooth_scale,
        y_scale,
        transpose_smooth_scale,
    ...
    yt_q, yt_scale = triton_transpose_rescale_smooth_quant(
        y_q,
        org_smooth_scale,
        y_scale,
        transpose_smooth_scale,
    ...
    output_check(yt_q_ref, yt_q[:, :M], "triton_transpose_rescale_smooth_quant.data")
    output_check(yt_scale_ref, yt_scale, "triton_transpose_rescale_smooth_quant.scale")
