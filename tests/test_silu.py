...
def torch_batch_weighted_silu_and_quant_forward(xs, weight, smooth_scales,
...
def torch_batch_weighted_silu_and_quant_backward(grad_output, x, weight,
    ...
    s = 0
    for i, c in enumerate(counts):
        q, scale = torch_smooth_quant(dx[s:s + c], grad_smooth_scales[i],
...
def test_silu_and_quant(M=4096, N=4096, bench=False):
    ...
    dx_q_ref, dx_scale_ref = torch_silu_and_quant_backward(grad_output, x,
                                                           grad_smooth_scale)
    dx_q, dx_scale = triton_silu_and_quant_backward(grad_output, x,
                                                    grad_smooth_scale,
                                                    reverse=False,
                                                    round_scale=True)
    ...
    if bench:
        benchmark_func(torch_silu_and_quant_forward, x, smooth_scale,
                       n_repeat=100, ref_bytes=M * N * 3)
        benchmark_func(triton_silu_and_quant_forward, x, smooth_scale,
...
def test_triton_batch_weighted_silu_and_quant(M=4096, N=4096, n_experts=32,
    ...
    round_scale = False
    x_q_ref, x_scale_ref = torch_batch_weighted_silu_and_quant_forward(x,
                                                                       weight,
                                                                       smooth_scales,
                                                                       counts,
                                                                       round_scale=round_scale,
                                                                       reverse=False)
    x_q, x_scale, _ = triton_batch_weighted_silu_and_quant_forward(x, weight,
                                                                   smooth_scales,
                                                                   counts,
                                                                   round_scale=round_scale,
                                                                   reverse=False)
    ...
    dx_ref, dx_scale_ref, dw_ref = torch_batch_weighted_silu_and_quant_backward(
        grad_output, x, weight, grad_smooth_scales, counts,
        round_scale=round_scale, reverse=False)
    dx, dx_scale, dw = triton_batch_weighted_silu_and_quant_backward(
        grad_output, x, weight, grad_smooth_scales, counts,
        round_scale=round_scale, reverse=False)
    ...
    if bench:
        ...
        benchmark_func(triton_batch_weighted_silu_and_quant_forward, x, weight,
                       smooth_scales, counts, round_scale=True, n_repeat=100,
        ...
        benchmark_func(triton_batch_weighted_silu_and_quant_backward,
                       grad_output, x, weight, smooth_scales, counts,
