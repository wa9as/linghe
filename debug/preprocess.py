...
if src == 'pkl':
    ...
    for i in range(1, 5):
        with safe_open(f"{model_path}/model-0000{i}-of-00004.safetensors",
    ...
    xb = torch.nn.functional.max_pool2d(torch.from_numpy(xp)[None], 11).numpy()[
        0]
    yb = torch.nn.functional.max_pool2d(torch.from_numpy(yp)[None], 11).numpy()[
        0]
    wb = torch.nn.functional.max_pool2d(torch.from_numpy(w)[None], 21).numpy()[
        0]
