...
def tp_moe_ref(hs, logits, ws, vs, n_act=4, rank=0, world_size=2):
    ...
    wss = [torch.empty((expert, 2, ims, dim), dtype=dtype, device=device) for _ in range(world_size)]
...
def ep_moe(hs, logits, ws, vs, n_act=4, rank=0, world_size=2, step=0):
    ...
    for i in range(exp):
        ...
        for j in range(world_size):
            ...
            expert_float_indices.append(expert_index.to(torch.int32)*step + expert*rank + idx)
            h = hs[expert_index]
            src_h_list.append(h)
        ...
        # 通信每个专家需要的索引
        dst_expert_float_indices = torch.empty([dst_token_count], dtype=torch.int32, device=device)
        dist.all_to_all_single(dst_expert_float_indices, expert_float_indices, output_split_sizes=dst_token_count_list, input_split_sizes=src_token_count_list)
    ...
    expert_indices_outputs = [torch.empty((c, ), dtype=torch.int32, device=device) for c in token_count_per_gpu]
...
def overlap_ep_moe(hs, logits, ws, vs, c_stream, d_stream, n_act=4, rank=0, world_size=2, step=0):
    # hs: [bs, length, dim]
    # logits: [bs, length, expert]
    # ws: [exp, im, dim]
    assert logits.dtype == torch.float32
...
if __name__ == '__main__':
    ...
    if mode in ('ep','tp'):
        ...
        if mode == 'ep':
            ...
            hidden_states = torch.ones((bs, length, dim), dtype=dtype, device=device)#*(rank+1)
            gate_up_weights = torch.ones((exp,2*ims,dim), dtype=dtype, device=device)#*(rank+1)
            down_weights = torch.ones((exp,dim, ims), dtype=dtype, device=device)#*(rank+1)
