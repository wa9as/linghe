...
@triton.jit
def reused_smooth_quant_tma_kernel(x_desc_ptr, q_desc_ptr, ss_ptr, qs_ptr, M, N,
    ...
    soffs = tl.arange(0, H)
    x_max = tl.zeros((W,),
                     dtype=tl.float32) + 5.27e-36  # torch.finfo(torch.float32).tiny*448
    n = tl.cdiv(N, H)
    for i in range(n):
        # x = tl.load(x_ptr+offs)
        offs_w = i * H
        x = tl._experimental_descriptor_load(x_desc_ptr, [offs, offs_w], [W, H],
                                             tl.float16)
...
def triton_reused_smooth_quant_tma(x, smooth_scale):
    ...
    triton.runtime.driver.active.utils.fill_2d_tma_descriptor(x.data_ptr(), M,
                                                              N, W, H,
                                                              x.element_size(),
                                                              desc_x)
    triton.runtime.driver.active.utils.fill_2d_tma_descriptor(x_q.data_ptr(), M,
                                                              N, W, H,
                                                              x_q.element_size(),
                                                              desc_xq)
