...
mode = 'channel'
if mode == 'channel':
    xq, wq, x_scale, w_scale = torch_duplex_smooth_quant(x, w, torch.float8_e4m3fn)
    xdq = xq.to(dtype) * x_scale
    wdq = wq.to(dtype) * w_scale
    opt_out = xdq @ wdq.t()
    quant_check(org_out, xq, wq, opt_out, 'torch_channel')

if mode == 'torch_channel_backward':
    yq, wq, y_scale, w_scale = torch_duplex_smooth_quant(y, w.t(), torch.float8_e4m3fn)
    ydq = yq.to(dtype) * y_scale
    wdq = wq.to(dtype) * w_scale
    opt_out = ydq @ wdq.t()
    quant_check(y @ w, yq, wq, opt_out, 'torch_channel_backward')

if mode == 'torch_channel_update':
    yq, xq, y_scale, x_scale = torch_duplex_smooth_quant(y.t(), x.t(),
                                                  torch.float8_e4m3fn)
