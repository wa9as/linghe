...
@triton.jit
def duplex_hadamard_nt_kernel(x_ptr, xb_ptr, xbt_ptr, w_ptr, wb_ptr, wbt_ptr,
                              hm_ptr, M, N, K, BLOCK_SIZE: tl.constexpr,
                              R: tl.constexpr):
    ...
    offs = pid * BLOCK_SIZE + tl.arange(0, R * BLOCK_SIZE)[:,
                              None] * K + tl.arange(0, BLOCK_SIZE)[None, :]
    toffs = pid * BLOCK_SIZE * N + tl.arange(0, BLOCK_SIZE)[:,
                                   None] * N + tl.arange(0, R * BLOCK_SIZE)[
                                               None, :]
...
@triton.jit
def duplex_hadamard_dy_kernel(y_ptr, yb_ptr, ybt_ptr, hm_ptr, M, N, K,
...
def duplex_hadamard_quant_forward(x, w, hm):
    x_bt, w_bt, x_q, w_q, x_scale, w_scale = triton_duplex_hadamard_quant_nt(x,
                                                                             w,
                                                                             hm)
