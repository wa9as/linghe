...
@triton.jit
def smooth_nt_kernel(x_ptr, xs_ptr, w_ptr, ws_ptr, smooth_scale_ptr, M, N, K,
...
# dx = y @wT
@triton.jit
def smooth_nn_kernel(y_ptr, ys_ptr, w_ptr, ws_ptr, M, N, K, H: tl.constexpr,
                     W: tl.constexpr):
    pid = tl.program_id(axis=0)
    ...
    w_scale = (w_max / scale).to(
        y_ptr.dtype.element_ty)  # reciprocal of x_scale
    offs = pid * W + tl.arange(0, H)[:, None] * N + tl.arange(0, W)[None, :]
    for i in range(m):
        y = tl.load(y_ptr + offs)
    ...
    y_scale = (y_max / scale).to(
        y_ptr.dtype.element_ty)  # reciprocal of w_scale
    offs = pid * W * K + tl.arange(0, W)[:, None] * K + tl.arange(0, H)[None, :]
...
# dwT = yT @ x
@triton.jit
def smooth_tn_kernel(y_ptr, ys_ptr, x_ptr, xs_ptr, M, N, K, H: tl.constexpr,
                     W: tl.constexpr):
    pid = tl.program_id(axis=0)
