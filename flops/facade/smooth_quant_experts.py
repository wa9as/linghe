...
class QuantExperts(torch.nn.Module):
    def __init__(
        ...
        self.gate_up_weight = torch.nn.parameter.Parameter(
            torch.empty((experts, 2 * ims, dim), device=device, dtype=dtype))
        self.down_weight = torch.nn.parameter.Parameter(
            torch.empty((experts, dim, ims), device=device, dtype=dtype))
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.training:
            if self.impl == 'naive':
                ...
                output, _ = _MixSmoothQuantExperts.apply(input, self.weight,
                                                         self.bias)
                return output
            elif self.impl == 'reused':
                if self.smooth_update_step % self.gap_step == 0:
                    output, smooth_scale = _ReusedSmoothQuantExperts.apply(
                        input, self.weight, self.bias, None)
                else:
                    output, smooth_scale = _ReusedSmoothQuantExperts.apply(
