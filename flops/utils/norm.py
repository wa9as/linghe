...
def rms_norm_and_block_quant_forward_kernel(x_ptr,
    ...
    for i in range(T):
        ...
        x = tl.reshape(x, [W, N])
        tl.store(scale_ptr + indices[:, None] * nb + tl.arange(0, nb)[None, :],
                 scale, mask=indices[:, None] < M)
...
@triton.jit
def group_norm_gate_forward_kernel(x_ptr, gate_ptr, weight_ptr, out_ptr, eps,
                                   bs, length,
                                   DIM: tl.constexpr,
                                   D: tl.constexpr,
    ...
    offs = sid * bs * DIM + bid * DIM + tl.arange(0, GROUP_SIZE)[:,
                                        None] * D + tl.arange(0, D)[
                                                    None, :]
...
def group_rms_gate_backward_kernel(
    ...
    x_offs = pid * DIM * T + tl.arange(0, GROUP_SIZE)[:, None] * D + tl.arange(
        0, D)[
                                                                     None, :]
    offs = sid * bs * DIM + bid * DIM + tl.arange(0, GROUP_SIZE)[:,
                                        None] * D + tl.arange(0, D)[
    ...
    for i in range(T):
        ...
        dx = r * g * w * gate - r * r * r * x * tl.sum(x * g * w * gate, 1,
                                                       keep_dims=True) / D
...
def triton_group_norm_gate_backward(grad_output, x, gate, weight, eps=1e-6,
                                    group_size=4):
    length, bs, dim = gate.shape
    assert dim <= 8192 and triton.next_power_of_2(
        dim) == dim and triton.next_power_of_2(group_size) == group_size
