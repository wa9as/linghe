...
def softmax_cross_entropy_backward_kernel(logit_ptr, label_ptr, sum_exp_ptr,
                                          ...
                                          est_max_logit, N, B: tl.constexpr):
    pid = tl.program_id(axis=0).to(tl.int64)
    label = tl.load(label_ptr + pid)
...
def triton_softmax_cross_entropy_backward(logits, labels, sum_exp, input_grad,
                                          output_grad=None, est_max_logit=0.0):
    M, N = logits.shape
    device = logits.device
