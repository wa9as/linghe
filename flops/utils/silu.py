...
def silu_and_block_quant_forward_kernel(x_ptr, 
    ...
    x = x1 * tl.sigmoid(x1.to(tl.float32)) * x2
...
@triton.jit
def compatible_silu_and_smooth_quant_backward_kernel(g_ptr, x_ptr, smooth_scale_ptr,
...
def silu_and_block_quant_backward_kernel(g_ptr, x_ptr,  
    ...
    x1 = tl.load(x_ptr + offs, mask=mask) #.to(tl.float32)
    x2 = tl.load(x_ptr + n + offs, mask=mask) #.to(tl.float32)
    g = tl.load(g_ptr + rid * 128 * n + cid * 128 + tl.arange(0, 128)[:, None] * n + tl.arange(0, 128)[None, :], mask=mask) #.to(tl.float32)
...
def triton_silu_and_quant_backward(g, x, smooth_scale=None, reverse=True,
    ...
    if smooth:
        if triton.next_power_of_2(N) == N and N <= 8192:
            ...
            grid = (M // T,)
            if smooth:
                compatible_silu_and_smooth_quant_backward_kernel[grid](
                    g,
                    x,
                    smooth_scale,
                    dx,
...
def batch_weighted_silu_and_smooth_quant_backward_kernel(g_ptr, x_ptr, weight_ptr,
    ...
    for i in range(c):
        ...
        dx2 = g * x1 * sigmoid * w
        if OUTPUT_GRAD:
            tl.store(grad_ptr + offs, dx1, mask=indices[:, None] < count)
            tl.store(grad_ptr + offs + n, dx2, mask=indices[:, None] < count)
...
def batch_weighted_silu_and_block_quant_backward_kernel(g_ptr, x_ptr, weight_ptr,
    ...
    if rid < tl.cdiv(count, 128):
        ...
        g = tl.load(g_ptr +  si * n + rid * 128 * n + 128 * cid + tl.arange(0, 128)[:, None] * n + tl.arange(0, 128)[None, :], mask=idx[:, None] < count) #.to(tl.float32)

        dw = tl.sum(x1 * tl.sigmoid(x1.to(tl.float32)) * x2 * g, 1)
        tl.store(dw_ptr + si * nb + cid + idx * nb, dw, mask=idx < count)

        # g = g * w

        dx = g * x2 * w * tl.sigmoid(x1.to(tl.float32)) * (
                1 + x1 * (1 - tl.sigmoid(x1.to(tl.float32))))
        scale = tl.maximum(
        ...
        tl.store(dx_ptr + offs, qdx, mask=idx[:, None] < count)
        ...
        tl.store(transpose_dx_ptr + si * n * 2 + rid * 128 + cid * 128 * count + tl.arange(0, 128)[:, None] * count + tl.arange(0, 128)[None, :], qdx, mask=idx[None, :] < count)

        # use recompute to save registers
        # sigmoid = 1 / (1 + tl.exp(-x1))
        # sigmoid = tl.sigmoid(x1)
        dx = g * x1 * tl.sigmoid(x1.to(tl.float32)) * w
        # dx = g * x1 * tl.sigmoid(x1)
        scale = tl.maximum(
            tl.max(dx.abs(), 1) / 448, 1e-30)
        if ROUND:
            scale = tl.exp2(tl.ceil(tl.log2(scale)))
        ...
        tl.store(dx_ptr + n + offs, qdx, mask=idx[:, None] < count)
...
def triton_batch_weighted_silu_and_quant_backward(g, x, weight, 
    ...
    if smooth:
        if output_grad:
            # output bf16 grad for quantized dxT
            grad = torch.empty((M, N), device=device, dtype=g.dtype) 
        else:
            grad = None
        transpose_dx = None 
        transpose_dx_scale = None 
    ...
    if smooth:
        ...
        dws = torch.empty((M, N//256), device=device, dtype=weight.dtype)
