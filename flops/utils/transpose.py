...
@triton.jit
def deprecated_transpose_kernel(x_ptr, t_ptr, M, N, H: tl.constexpr, W: tl.constexpr, EVEN: tl.constexpr):
...
@triton.jit
def transpose_and_pad_kernel(x_ptr, t_ptr, M, N, P, H: tl.constexpr, W: tl.constexpr, EVEN: tl.constexpr):
    rid = tl.program_id(axis=0)
    cid = tl.program_id(axis=1)
    offs = rid*H*N + cid*W + tl.arange(0, H)[:,None]*N + tl.arange(0, W)[None,:] 
...
@triton.jit
def batch_transpose_and_pad_kernel(x_ptr, t_ptr, count_ptr, accum_ptr, pad_accum_ptr, N, H: tl.constexpr, W: tl.constexpr):
    eid = tl.program_id(axis=0)
    cid = tl.program_id(axis=1)
