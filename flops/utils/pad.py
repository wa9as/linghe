...
@triton.jit
def slice_and_pad_kernel(x_ptr, index_ptr, out_ptr, DIM: tl.constexpr,
                         BLOCK: tl.constexpr):
...
def triton_slice_and_pad(x, indices, block=32):
    ...
    size = indices.size(0)
    device = x.device
    output = torch.empty((((size - 1) // block + 1) * block, dim),
...
@triton.jit
def batch_slice_and_pad_kernel(x_ptr, indices_ptrs, size_ptr, out_ptrs,
                               GROUP: tl.constexpr, DIM: tl.constexpr,
                               BLOCK: tl.constexpr):
    pid = tl.program_id(axis=0)
    indices = tl.arange(0, BLOCK)
    for g in range(GROUP):
        ...
        if pid < size:
            ...
            o_ptr = tl.load(out_ptrs + g).to(
                tl.pointer_type(x_ptr.dtype.element_ty))
            b = DIM // BLOCK
...
def triton_batch_slice_and_pad(x, indices, expert=64, block=32):
    ...
    max_size = max(sizes)
    outputs = [
        torch.empty((((size - 1) // block + 1) * block, dim), device=device,
                    dtype=x.dtype) for size in sizes]
    indices_ptrs = torch.tensor([i.data_ptr() for i in indices],
                                device=x.device)
