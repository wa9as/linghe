...
@triton.jit
def silu_and_block_quant_forward_kernel(
    x_ptr,
    out_ptr,
    scale_ptr,
    ...
    if OUTPUT_MODE % 2 == 0:
        ...
        tl.store(
            scale_ptr + rid * 128 + cid * M + tl.arange(0, 128), scale, mask=indices < M
        )
...
def triton_silu_and_block_quant_forward(
    x, out=None, scale=None, round_scale=False, output_mode=2
):
    """
    ...
    transpose_output = torch.empty(
        (N // 2, M), device=device, dtype=torch.float8_e4m3fn
    )
    transpose_scale = torch.empty(
        (triton.cdiv(M, 128), N // 2), device=device, dtype=torch.float32
    )
...
@triton.jit
def silu_and_block_quant_backward_kernel(
    g_ptr,
    x_ptr,
    dx_ptr,
    dx_scale_ptr,
    ...
    dx1 = (
        sigmoid * g * x2 * (1 + x1 * (1 - sigmoid))
    )  # change order to trigger autocast
    scale1 = tl.maximum(tl.max(dx1.abs(), 1) / 448, 1e-30)
    if ROUND:
        scale1 = tl.exp2(tl.ceil(tl.log2(scale1)))
    tl.store(
        dx_scale_ptr + cid * M + rid * 128 + tl.arange(0, 128), scale1, mask=idx < M
    )

    qdx1 = (dx1 / scale1[:, None]).to(dx_ptr.dtype.element_ty)
    tl.store(dx_ptr + offs, qdx1, mask=mask)

    scale1 = tl.maximum(tl.max(dx1.abs(), 0) / 448, 1e-30)
    if ROUND:
        scale1 = tl.exp2(tl.ceil(tl.log2(scale1)))
    tl.store(
        transpose_dx_scale_ptr + rid * n * 2 + cid * 128 + tl.arange(0, 128), scale1
    )
    ...
    dx2 = sigmoid * g * x1
    scale2 = tl.maximum(tl.max(dx2.abs(), 1) / 448, 1e-30)
    if ROUND:
        scale2 = tl.exp2(tl.ceil(tl.log2(scale2)))
    tl.store(
        dx_scale_ptr + cid * M + rid * 128 + M * nb + tl.arange(0, 128),
        scale2,
        mask=idx < M,
    )
    ...
    scale2 = tl.maximum(tl.max(dx2.abs(), 0) / 448, 1e-30)
    if ROUND:
        scale2 = tl.exp2(tl.ceil(tl.log2(scale2)))
    tl.store(
        transpose_dx_scale_ptr + rid * n * 2 + n + cid * 128 + tl.arange(0, 128), scale2
    )
...
@triton.jit
def batch_weighted_silu_and_block_quant_forward_kernel(
    x_ptr,
    weight_ptr,
    out_ptr,
    scale_ptr,
    ...
    if OUTPUT_MODE % 2 == 0:
        ...
        if ROUND:
            scale = tl.exp2(tl.ceil(tl.log2(scale)))
        tl.store(
            scale_ptr + si * nb + cid * count + rid * 128 + tl.arange(0, 128),
            scale,
            mask=indices < count,
        )
...
@triton.jit
def batch_weighted_silu_and_block_quant_backward_kernel(
    g_ptr,
    x_ptr,
    weight_ptr,
    count_ptr,
    ...
    nb = n // 128
    transpose_off = tl.sum(
        tl.where(
            tl.arange(0, E) < eid, tl.cdiv(tl.load(count_ptr + tl.arange(0, E)), 128), 0
        )
    )
    ...
    scale = tl.maximum(tl.max(dx.abs(), 1) / 448, 1e-30)
    if ROUND:
        scale = tl.exp2(tl.ceil(tl.log2(scale)))
    tl.store(
        dx_scale_ptr + si * nb * 2 + cid * count + rid * 128 + tl.arange(0, 128),
        scale,
        mask=idx < count,
    )
    ...
    scale = tl.maximum(tl.max(dx.abs(), 0) / 448, 1e-30)
    if ROUND:
        scale = tl.exp2(tl.ceil(tl.log2(scale)))
    tl.store(
        transpose_dx_scale_ptr
...
def triton_batch_weighted_silu_and_block_quant_backward(
    g, x, weight, counts, splits=None, round_scale=False
):
    """
...
# n is power of 2
@triton.jit
def silu_and_smooth_quant_forward_kernel(
    x_ptr,
    smooth_scale_ptr,
    out_ptr,
    scale_ptr,
...
# n is NOT power of 2
@triton.jit
def compatible_silu_and_smooth_quant_forward_kernel(
    x_ptr,
    smooth_scale_ptr,
    out_ptr,
    scale_ptr,
...
# used in shared expert
def triton_silu_and_smooth_quant_forward(
    x,
    smooth_scale=None,
    out=None,
    scale=None,
...
@triton.jit
def silu_and_smooth_quant_backward_kernel(
    g_ptr,
    x_ptr,
    smooth_scale_ptr,
    ...
    offs = pid * T * n * 2 + tl.arange(0, T)[:, None] * n * 2 + tl.arange(0, B)[None, :]
    ...
    transpose_smooth_scale = tl.load(
        transpose_smooth_scale_ptr + pid * T + tl.arange(0, T)
    )[:, None]
    for i in range(nb):
        ...
        t_dx = t_dx / t_s
        tl.store(
            transpose_dx_ptr + toffs,
            tl.trans(t_dx.to(transpose_dx_ptr.dtype.element_ty)),
        )
        ...
        t_dx = t_dx / t_s
        tl.store(
            transpose_dx_ptr + M * n + toffs,
            tl.trans(t_dx.to(transpose_dx_ptr.dtype.element_ty)),
        )
        tl.store(
            transpose_dx_scale_ptr + pid * n * 2 + n + i * B + tl.arange(0, B), t_s
        )
        ...
        maxs = tl.maximum(tl.maximum(tl.max(dx1.abs(), 1), tl.max(dx2.abs(), 1)), maxs)
    ...
    for i in range(nb):
        ...
        g = tl.load(g_ptr + hoffs).to(tl.float32)
        sigmoid = 1 / (1 + tl.exp(-x1))
        dx1 = g * x2 * sigmoid * (1 + x1 * (1 - sigmoid)) * smooth_scale_1
...
@triton.jit
def _requant_kernel(
    x_ptr, scale_ptr, scales_ptr, M, N, H: tl.constexpr, W: tl.constexpr
):
    rid = tl.program_id(axis=0)
    cid = tl.program_id(axis=1)
    offs = (
...
# used in shared expert
def triton_silu_and_smooth_quant_backward(
    g,
    x,
    smooth_scale=None,
    ...
    grid = (N // B, M // T)
    _requant_kernel[grid](
        transpose_dx, transpose_dx_scale, transpose_dx_scales, N, M, B, T
    )
...
# used in routed experts
def triton_batch_weighted_silu_and_smooth_quant_forward(
    x,
    weight,
    counts,
    smooth_scale=None,
    ...
    if scale is None:
        scale = torch.empty((M,), device=device, dtype=torch.float32)
    if M == 0:
        maxs = torch.zeros((n_experts, n), device=device, dtype=torch.float32)

    elif calibrate:
        tmp_maxs = torch.empty((n_experts, sm, n), device=device, dtype=torch.float32)
        maxs = torch.empty((n_experts, n), device=device, dtype=torch.float32)
    else:
        maxs = None
...
@triton.jit
def batch_weighted_silu_and_smooth_quant_backward_kernel(
    g_ptr,
    x_ptr,
    weight_ptr,
    smooth_scale_ptr,
    ...
    round_off = (
        tl.sum(
            tl.where(
                tl.arange(0, E) < eid,
                tl.cdiv(tl.load(count_ptr + tl.arange(0, E)), 32),
                0,
            )
        )
        * 32
    ...
    offs = (
        si * n * 2
        + pid * T * n * 2
        + tl.arange(0, T)[:, None] * n * 2
        + tl.arange(0, B)[None, :]
    )
    hoffs = (
        si * n + pid * T * n + tl.arange(0, T)[:, None] * n + tl.arange(0, B)[None, :]
    )
    toffs = (
        round_off * n * 2
        + pid * T
        + tl.arange(0, B)[:, None] * round_count
        + tl.arange(0, T)[None, :]
    )
    nb = n // B
    maxs = tl.zeros((T,), dtype=tl.float32)
    ...
    if REVERSE:
        transpose_smooth_scale = tl.load(
            transpose_smooth_scale_ptr + si + pid * T + tl.arange(0, T),
            mask=indices < count,
        )[:, None]
    else:
        transpose_smooth_scale = (
    ...
    w = tl.load(weight_ptr + si + pid * T + tl.arange(0, T), mask=indices < count)[
        :, None
    ]
    dw = tl.zeros((T,), dtype=tl.float32)
    qdtype = transpose_dx_ptr.dtype.element_ty
    for i in range(nb):
        smooth_scale_1 = tl.load(
            smooth_scale_ptr + eid * n * 2 + i * B + tl.arange(0, B)
        )
        smooth_scale_2 = tl.load(
            smooth_scale_ptr + eid * n * 2 + n + i * B + tl.arange(0, B)
        )
        if not REVERSE:
            smooth_scale_1 = 1 / smooth_scale_1
            smooth_scale_2 = 1 / smooth_scale_2
        ...
        sigmoid = 1 / (1 + tl.exp(-x1))
        dx1 = g * x2 * sigmoid * (1 + x1 * (1 - sigmoid)) * w
        dx2 = g * x1 * sigmoid * w
        ...
        t_dx = t_dx / t_s
        tl.store(
            transpose_dx_ptr + round_count * n + toffs,
            tl.trans(t_dx.to(qdtype)),
            mask=indices[None, :] < round_count,
        )
        tl.store(
    ...
    for i in range(nb):
        ...
        g = tl.load(g_ptr + hoffs, mask=indices[:, None] < count).to(tl.float32)
        sigmoid = 1 / (1 + tl.exp(-x1))
        dx1 = g * x2 * sigmoid * (1 + x1 * (1 - sigmoid)) * smooth_scale_1 * w
...
def _batch_requant_kernel(
    ...
    round_off = tl.sum(
        tl.where(
            tl.arange(0, E) < eid,
            tl.cdiv(tl.load(count_ptr + tl.arange(0, E)), 32) * 32,
            0,
        )
    )
    ...
    offs = (
        ...
        + rid * H * round_count
        + cid * W
        + tl.arange(0, H)[:, None] * round_count
        + tl.arange(0, W)[None, :]
    )
    global_scale = tl.load(scale_ptr + eid * N + rid * H + tl.arange(0, H))
    # scales is stored with column-major format
    local_scale = tl.load(
        scales_ptr + max_block * N * eid + cid * N + rid * H + tl.arange(0, H)
    )
    x = tl.load(x_ptr + offs).to(tl.float32)
...
# used in routed experts
def triton_batch_weighted_silu_and_smooth_quant_backward(
    g,
    x,
    weight,
    counts,
    ...
    if s == 0:
        transpose_dx_scale = torch.zeros(
            (n_expert, N), device=device, dtype=torch.float32
        )
        return dx, dx_scale, dw, transpose_dx, transpose_dx_scale
    else:
        transpose_dx_scales = torch.zeros(
            (n_expert, max_block, N), device=device, dtype=torch.bfloat16
        )
