...
def benchmark_with_shape(shape):
    ...
    ref_time = benchmark_func(fp16_forward, x, w.t(), n_repeat=n_repeat,
                              ref_flops=M * K * N * 2)
    benchmark_func(smooth_quant_forward, x, w, n_repeat=n_repeat,
                   ref_flops=M * K * N * 2, ref_time=ref_time)
    ref_time = benchmark_func(fp16_backward, y, w, n_repeat=n_repeat,
                              ref_flops=M * K * N * 2)
    benchmark_func(smooth_quant_backward, y, w, n_repeat=n_repeat,
                   ref_flops=M * K * N * 2, ref_time=ref_time)
    ref_time = benchmark_func(fp16_update, y, x, n_repeat=n_repeat,
                              ref_flops=M * K * N * 2)
    benchmark_func(smooth_quant_update, y, x, n_repeat=n_repeat,
                   ref_flops=M * K * N * 2, ref_time=ref_time)
    ...
    ref_time = benchmark_func(fp16_f_and_b, x, w, y, n_repeat=n_repeat,
                              ref_flops=M * K * N * 6)
    benchmark_func(reused_smooth_quant_f_and_b, x, w, y, n_repeat=n_repeat,
                   ref_flops=M * K * N * 6, ref_time=ref_time)


# benchmark_with_shape([2048-1, 8192, 2048])
# benchmark_with_shape([8192, 8192, 2048])

for shape in [[8192, 10240, 8192], [8192, 8192, 8192], [8192, 4096, 8192],
              [8192, 8192, 2048], [2048, 4096, 8192], [2048, 8192, 2048]]:
    shape[0] -= 1
