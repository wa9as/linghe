...
def benchmark_with_shape(shape):
    ...
    benchmark_func(triton_hadamard_quant_nt_megatron, x, w, hm,
                   n_repeat=n_repeat)
    benchmark_func(triton_hadamard_quant_nn, y, w, hm, n_repeat=n_repeat)
    benchmark_func(triton_hadamard_quant_nn_megatron, y, w, hm,
                   n_repeat=n_repeat)
    benchmark_func(triton_hadamard_quant_tn, y, x, hm, n_repeat=n_repeat)
    benchmark_func(triton_hadamard_quant_tn_megatron, y, x, hm,
                   n_repeat=n_repeat)
    ...
    ref_time = benchmark_func(fp16_f_and_b, x, w, y, n_repeat=n_repeat,
                              ref_flops=M * K * N * 6)
    benchmark_func(fp8_hadamard_f_and_b, x, w, y, hm, n_repeat=n_repeat,
                   ref_time=ref_time, ref_flops=M * K * N * 6)
    benchmark_func(fp8_fused_hadamard_f_and_b, x, w, y, hm, n_repeat=n_repeat,
                   ref_time=ref_time, ref_flops=M * K * N * 6)
    benchmark_func(fp8_hadamard_f_and_b_megatron, x, w, y, hm,
                   n_repeat=n_repeat, ref_time=ref_time,
                   ref_flops=M * K * N * 6)
    benchmark_func(fp8_duplex_hadamard_f_and_b, x, w, y, hm, n_repeat=n_repeat,
                   ref_time=ref_time, ref_flops=M * K * N * 6)
