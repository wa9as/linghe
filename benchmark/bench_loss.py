...
def triton_cross_entropy_forward_backward(logits, targets, input_grad):
    ...
    output_grad = triton_softmax_cross_entropy_backward(
        logits, targets, sum_exp, max_logits, input_grad
    )
...
def bench_triton_softmax_cross_entropy(M=4096, N=157184):
    device = "cuda:0"
    logits = (
        torch.randn((M, N), dtype=torch.bfloat16, device=device, requires_grad=True)
        ** 1
    )
