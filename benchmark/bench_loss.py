...
def triton_cross_entropy_forward_backward(logits, targets, input_grad):
    losses, sum_exp, max_logits = triton_softmax_cross_entropy_forward(logits, targets)
...
def bench_triton_softmax_cross_entropy(M=4096, N=157184):
    ...
    torch_losses, torch_grad = torch_cross_entropy(logits.detach().clone().requires_grad_(), targets)
    fused_losses, fused_grad = fused_cross_entropy_forward_backward(logits.detach().clone().requires_grad_(), targets, pg)
    triton_losses, sum_exp_, max_logit_ = triton_softmax_cross_entropy_forward(logits, targets)
    output_check(torch_losses, triton_losses)
    output_check(fused_losses, triton_losses)
    output_check(torch_losses, fused_losses)

    ref_time = benchmark_func(torch_cross_entropy, logits.detach().clone().requires_grad_(), targets,
                              ref_bytes=M * N * 4)
    ...
    benchmark_func(te_cross_entropy_forward_backward, logits.detach().clone().requires_grad_(), targets, ref_bytes=M * N * 4,
                   ref_time=ref_time)
    benchmark_func(te_cross_entropy_forward_backward, logits.detach().clone().requires_grad_(), targets, ref_bytes=M * N * 4,
                   ref_time=ref_time)
