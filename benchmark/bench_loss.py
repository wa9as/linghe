...
def bench_triton_softmax_cross_entropy(M=4096, N=157184):
    ...
    benchmark_func(triton_softmax_cross_entropy_backward, logits, targets,
                   sum_exp, input_grad, ref_bytes=M * N * 4, ref_time=ref_time)
    benchmark_func(parallel_cross_entropy, logits[None], targets[None],
                   ref_bytes=M * N * 4, ref_time=ref_time)
    benchmark_func(te_cross_entropy, logits, targets, ref_bytes=M * N * 4,
                   ref_time=ref_time)
